...
5. If the diff touches `packages/core/`, `agent/loop.py`, `schemes/extract_rules.py`, `interview/guard.py`, or `voice/latency.py` — dispatch `reviewer`. Fix every finding before continuing.
6. If the diff touches `apps/web/` — dispatch `designer` for the critique pass.
7. Run the full gate: `cd packages/core && uv run pytest -q`, `cd apps/api && uv run pytest -q`, `cd apps/web && pnpm test`, `uv run python ../../evals/run.py`. Show real output. A failing test is never skipped.
